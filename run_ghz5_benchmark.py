"""GHZ-5 benchmark with N=20000, 10 replicates, 3 noise profiles + raw shots.

Usage:
    python run_ghz5_benchmark.py
"""
...
OUTPUT_BASE = "pilot_analysis_noise_ghz5"
SEED = 42
N_QUBITS = 5
...
def build_merged_observable_set(n_qubits: int) -> tuple[ObservableSet, dict[str, int]]:
    ...
    obs_set = ObservableSet(
        observables=merged,
        observable_set_id=f"GHZ{n_qubits}_merged",
...
def main():
    ...
    obs_set, loc_map = build_merged_observable_set(N_QUBITS)

    print(f"Circuit: GHZ-{N_QUBITS} ({qc.num_qubits} qubits)")
    ...
    for noise_profile in NOISE_PROFILES:
        ...
        result = run_benchmark_suite(
            ...
            circuit_id=f"GHZ{N_QUBITS}__{noise_profile}",
            config=config,
