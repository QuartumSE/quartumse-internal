"""C-H2 benchmark with N=20000, 10 replicates, 3 noise profiles + raw shots.

Runs the same setup as notebook_research_benchmark.ipynb but with
store_raw_shots=True for post-hoc pilot analysis.

Usage:
    python run_noise_benchmark.py
"""
...
OUTPUT_BASE = "pilot_analysis_noise"
SEED = 42


def build_h2_circuit() -> QuantumCircuit:
    qc = QuantumCircuit(4)
    qc.x(0)
    qc.x(1)
    qc.ry(np.pi / 6, 2)
...
def main():
    ...
    print(f"Circuit: C-H2 ({qc.num_qubits} qubits)")
    ...
    for noise_profile in NOISE_PROFILES:
        ...
        result = run_benchmark_suite(
            ...
            circuit_id=f"C-H2__{noise_profile}",
            config=config,
